      ...
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
          ...
          <div className="text-sm text-slate-600">
            {t("results.pointsPlayed", {
              count: String(distribution.totalPoints),
            })}
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {teamDistribution.map((td) => (
                <div key={td.team} className="space-y-2">
                  <div
                    className={`text-sm font-bold ${td.team === 1 ? "text-team1" : "text-team2"}`}
                  >
                    {td.team === 1 ? team1Label : team2Label} —{" "}
                    {td.totalPointsWon} pts
                  </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-slate-500">
                <div className="flex items-center gap-1">
                  ...
                  W ({distribution.decidedByWinner})
                </div>
                <div className="flex items-center gap-1">
        ...
        {magiaTeamStats &&
          magiaPlayerStats &&
          magiaTeamStats.some((tm) => tm.total > 0) && (
            <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
              <h2 className="font-semibold text-slate-800">
                ...
                {magiaTeamStats.map((tm) => (
                  ...
                  >
                    ...
                    <div className="space-y-0.5 text-sm">
                      {magiaTypes
                        .filter((mt) => tm.byType[mt] > 0)
                        .map((mt) => (
                          <div key={mt} className="flex justify-between">
                            <span className="text-slate-600">
                              {MAGIA_TYPE_LABELS[mt]}
                            </span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-slate-500 text-xs">
                      ...
                      {magiaTypes.map((mt) => (
                        <th key={mt} className="text-center py-2 px-1">
                          {magiaShortLabels[mt]}
                        </th>
                      ))}
                    ...
                    {magiaPlayerStats
                      ...
                      .map((ps, i) => (
                        <tr key={i} className="border-t border-slate-100">
                          <td className="py-2 pr-2">
                            <span
                              className={`font-medium ${ps.team === 1 ? "text-team1" : "text-team2"}`}
                            >
                              {ps.playerName}
                            </span>
                          </td>
